...
const utils = {
  ...
  delegatedEventListener: function(matchingStrategy, onCallback) {
    return evt => matchingStrategy(evt) ? onCallback.call(evt.target, evt) : null;
  },
  /**
  ...
  selectorMatchingStrategy: function(selector) {
    return (evt) => evt.target.matches(selector) || !!evt.target.closest(selector);
  },
  /**
  ...
  delegatedTargetSelectorEventListener: function(selector, onCallback) {
    return this.delegatedEventListener(this.selectorMatchingStrategy(selector), onCallback);
  },
  /**
...
(() => {
   ...
   */
  document.querySelector('.nav__links').addEventListener('click', utils.delegatedTargetSelectorEventListener(
    navLinkSel,
    (evt) => {
      evt.preventDefault();
  ...
  function handleHover(opacity) {
    return function(evt) {
      nav.querySelectorAll(navLinkSel).forEach(el => el !== evt.target ? el.style.opacity = `${opacity}` : null);
      nav.querySelector('img').style.opacity = `${opacity}`;
    };
  }

  nav.addEventListener('mouseover', utils.delegatedTargetSelectorEventListener(navLinkSel, handleHover(0.5)));
  nav.addEventListener('mouseout', utils.delegatedTargetSelectorEventListener(navLinkSel, handleHover(1)));
...
(() => {
  ...
  const tabContainer = operationsZone.querySelector('.operations__tab-container');
  ...
  tabContainer.addEventListener('click', utils.delegatedTargetSelectorEventListener(
    tabBtnSelector,
    function() {
      const clickedBtn = this.closest(tabBtnSelector);

      tabButtons.forEach(btn => btn.classList.remove('operations__tab--active'));
      tabs.forEach(zone => zone.classList.remove('operations__content--active'));

      clickedBtn.classList.add('operations__tab--active');
      operationsZone.querySelector(`.operations__content--${clickedBtn.dataset.tab}`)
...
(() => {
  ...
  function goTo(selected) {
    ...
    slides.forEach((slide, index) => slide.style.transform = `translateX(${(index - selected) * 100}%)`);
    // 3 - Update the dots active class
    dots.querySelectorAll(`.${dotClassName}`).forEach((dot, index) =>
   ...
   */
  function addDots() {
    dots.innerHTML = Array.from(slides.values()).reduce((html, _, index) =>
  ...
  slider.querySelector('.slider__btn--left').addEventListener('click', previous);
  ...
  // By dots button
  slider.addEventListener('click', utils.delegatedTargetSelectorEventListener(
    `.${dotClassName}`,
    (evt) => goTo(parseInt(evt.target.dataset.slide))
  ));
})();